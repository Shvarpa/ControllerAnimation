<script context="module">
	...
	import xboxImageConfig from "../resources/xboxImageConfig";
...
<script>
	...
	export let userIndex = undefined;

	$: config = config == undefined ? defaultConfig : config;

	const drawUserIndex = canvas => {
		if (!userIndex || typeof userIndex != "number" || userIndex > 4 || userIndex < 1) return;
		let { width, height } = canvas;
		let { x, y, size } = xboxImageConfig.buttons.GUIDE;
		let ctx = canvas.getContext("2d");
		restore(ctx, ["strokeStyle", "globalAlpha"])(() => {
			ctx.strokeStyle = "rgb(123,187,100)";
			ctx.globalAlpha = 1;
			ctx.beginPath();
			ctx.arc(x * width, y * height, size * height, ...ranges[userIndex - 1]);
			ctx.stroke();
			ctx.closePath();
		});
	};
...
<Controller
	...
	{showText}
	{picker}
	controllerIconSrc="{xboxSvg}"
	imageConfig="{xboxImageConfig}"
	controllerConfig="{config}"
	detectionScale="{1.3}"
	axisRadiusScale="{0.08}"
	addons="{[drawUserIndex]}"
	on:click />
